...
def check(LT, LT_end):
    ...
    document = load_file("uploads/konto.pdf") 
    ...
    df = pd.read_excel("uploads/KT_konto.xlsx") 
    ...
    for x in range(len(DL_excel)):
        if DL_excel[x][2] in [y[2] for y in DL_pdf]: 
            ...
            if Qty_check_list[x][0] == Qty_check_list[x][-1]:
                ...
                Qty_diff.append([Qty_check_list[x],DL_excel[x][0]])
            
        else:
            Their_not_found.append(DL_excel[x])
    ...
    for x in range(len(DL_pdf)):
        if DL_pdf[x][2] in [y[2] for y in DL_excel]: 
            ...
            Our_not_found.append(DL_pdf[x])

    return (Qty_diff, Their_not_found, Our_not_found)
